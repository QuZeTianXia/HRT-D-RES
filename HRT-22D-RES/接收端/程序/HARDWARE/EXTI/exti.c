...
void EXTI9_5_IRQHandler(void)
{
   ...
   if(EXTI_GetITStatus(EXTI_Line6)!=RESET)//验证EXIT LINE 6 中断信号
     {
			 
					 
        EXTI_ClearITPendingBit(EXTI_Line6);  //清除EXIT LINE中断标志位
     }
		 
	 if(EXTI_GetITStatus(EXTI_Line7)!=RESET)//验证EXIT LINE 7 中断信号
     {
       if(0)
          OLED_ShowChar(0,16,'#',12,1);
			 else
			  	OLED_ShowChar(0,16,' ',12,1);		
			 		 
        EXTI_ClearITPendingBit(EXTI_Line7);  //清除EXIT LINE中断标志位
     }
