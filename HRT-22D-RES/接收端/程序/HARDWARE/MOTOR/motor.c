...
{		 		
  ...
  MiniBalance_Motor_Init();	
	...
	TIM_OCInitStructure.TIM_Pulse = 1331;                          	  //设置待装入捕获比较寄存器的脉冲值
...
{  
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_AFIO,ENABLE);								//功能复用时钟
  ...
  //	GPIO_Init(GPIOB, &GPIO_InitStructure);
	...
	TIM_TimeBaseStructure.TIM_Period = 1439;	//arr 设置在下一个更新事件装入活动的自动重装载寄存器周期的值	 80K
	TIM_TimeBaseStructure.TIM_Prescaler = 999;	//设置用来作为TIMx时钟频率除数的预分频值  不分频
	TIM_TimeBaseStructure.TIM_ClockDivision = 0;	//设置时钟分割:TDTS = Tck_tim
	TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up;	//TIM向上计数模式
